...
def generate_image(true_dist):
    ...
    plt.savefig('sam_tmp/' + DATASET + '/' + 'frame' + str(frame_index[0]) + '.jpg')
...
for iteration in range(ITERS):
    ...
    if not FIXED_GENERATOR:
        ...
        transformed_noise = netNM(noisev)
        fake = netG(transformed_noise)
        G = netD(fake)
        G = G.mean()
        G_cost = -G
        G.backward(mone)
    ...
    for p in netNM.parameters():
        p.requires_grad = True
    for p in netG.parameters():
        p.requires_grad = True #For backprop pui

    netG.zero_grad()
    netNM.zero_grad()

    # This one we optimize the NM...
    _data = next(data)
    real_data = torch.Tensor(_data)
    if use_cuda:
        real_data = real_data.cuda()
    ...
    G = netD(fake)
    G = G.mean()
    G.backward(one) #I think this should be one, because it's the opposite of the Generator...
    # G.backward(mone) #NOTE: THIS IS BAD!!!!
    # TODO: Is the NM even doing anything? It could just not be optimizing...
    optimizerNM.step()
    ...
    lib.plot.plot('sam_tmp/' + DATASET + '/' + 'disc cost', D_cost.cpu().data.numpy())
    lib.plot.plot('sam_tmp/' + DATASET + '/' + 'wasserstein distance', Wasserstein_D.cpu().data.numpy())
    if not FIXED_GENERATOR:
        lib.plot.plot('sam_tmp/' + DATASET + '/' + 'gen cost', G_cost.cpu().data.numpy())
    # if iteration % 100 == 99:
